...
##																											##
## Copyright (c) 2025, Pascal Sasdrich. All rights reserved.												##
##############################################################################################################
...
INCLUDE_DIR			:= ./inc
LIBRARY_DIR			:= ./lib
CUDD_LIBRARY_DIR	:= /usr/local/lib/
BINARY_DIR			:= ./bin
BUILD_DIR			:= ./build
...
C_FULLTEST_FLAGS	:= -m64 -fopenmp -march=native -O3 -fomit-frame-pointer -fprofile-arcs -ftest-coverage -std=c11 -D UNITTEST -D FULLTEST
C_MERGETEST_FLAGS	:= -m64 -fopenmp -march=native -O3 -fomit-frame-pointer -fprofile-arcs -ftest-coverage -std=c11 -D UNITTEST -D MERGETEST
...
CXX_FULLTEST_FLAGS	:= -m64 -fopenmp -march=native -O3 -fomit-frame-pointer -fprofile-arcs -ftest-coverage -std=c++17 -D UNITTEST -D FULLTEST
CXX_MERGETEST_FLAGS	:= -m64 -fopenmp -march=native -O3 -fomit-frame-pointer -fprofile-arcs -ftest-coverage -std=c++17 -D UNITTEST -D MERGETEST

LD_RELEASE_FLAGS	:= -m64 -fopenmp -Wl,-rpath=$(LIBRARY_DIR) -Wl,-rpath=$(CUDD_LIBRARY_DIR)
...
# LIBRARIES
LIB_RELEASE			:= -lcudd -I ./include/boost/ -lboost_program_options
...
$(OBJECT_DIR)/%.o: $(SOURCE_DIR)/%.c
	@mkdir -p '$(dir $@)'
	$(SUPPRESS)$(CC) $(DEPENDENCYFLAGS) $(C_FLAGS) $(INCLUDES) -o $@ -c $<
	@touch $@

# compile .cpp files
$(OBJECT_DIR)/%.o: $(SOURCE_DIR)/%.cpp
	@mkdir -p '$(dir $@)'
	$(SUPPRESS)$(CXX) $(DEPENDENCYFLAGS) $(CXX_FLAGS) $(INCLUDES) -o $@ -c $<
	@touch $@
